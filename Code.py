...
13 
l=[] 
#extracting companies that have complete weekday data for i in range(0,X.shape[0]): 
 if X[k][6]==X[i][6]: 
 j1=j1+1  
 else: 
 if (j1==1258): 
...
for i in list(dict_X.keys()): 
 ...
 dict_dataset_comp[len(X_dataset_comp)-1]=X[i][X.shape[1]-1]  l=[] 
#extracting oil prices
14 
oil_prices_dataset=pd.read_csv('oil_prices.csv') 
Y=oil_prices_dataset.iloc[:,:].values 
date_X=list(dataset_all_stocks.iloc[0:len(X_dataset_comp[0]),0].values) date_X=set(date_X) 
date_X=list(date_X) 
...
for i in range(0,len(date_X)): 
 ...
 date_unequal_index.append(date_Y.index(date_Y[i]))  date_Y.remove(date_Y[i]) 
...
exchange_dataset=pd.read_csv('exchange_rate.csv')  e=exchange_dataset.iloc[:,:].values 
j=0 
...
for i in range(0,len(X_dataset_comp)): 
 X_dataset_comp[i]=np.hstack((X_dataset_comp[i],list1_oil_price_add))  X_dataset_comp[i]=np.hstack((X_dataset_comp[i],list1_exchange_values)) 
...
for i in range(0,len(X_dataset_comp)): 
 X_dataset_comp[i]=np.hstack((X_dataset_comp[i],a_year))  X_dataset_comp[i]=np.hstack((X_dataset_comp[i],a_month))  X_dataset_comp[i]=np.hstack((X_dataset_comp[i],a_day)) 
...
fedfund_d=pd.read_csv('FEDFUNDS.csv') fedfund=fedfund_d.iloc[:,:].values l_fedfund_index=[] 
k=0 
for i in range(0,6): 
 l_year=[] 
 for j in range(0,12): 
 ...
 l_year.append(fedfund[k][1])  k=k+1 
 l_fedfund_index.append(l_year) l_fedfund_index[5].append(1.42) 
#merging CPI 
l_cpi_values=[]
19 
for i in range(0,len(X_dataset_comp)): 
 ...
 X_dataset_comp[i]=np.hstack((X_dataset_comp[i],l_cpi_values))  l_cpi_values=[] 
...
for i in range(0,len(X_dataset_comp)): 
 ...
 X_dataset_comp[i]=np.hstack((X_dataset_comp[i],l_fedfund_values))  l_fedfund_values=[] 
#data preprocessing 
from sklearn.preprocessing import StandardScaler,Imputer im=Imputer(strategy='mean',axis=1)
20 
ss=StandardScaler() 
dict_comp={} 
Y_dataset_comp=[] 
...
#classifier part 
from sklearn.ensemble import GradientBoostingRegressor gbregressor=GradientBoostingRegressor(n_estimators=100,random_state=0) gbregressor.fit(X_train,Y_train) 
...
for i in range(0,len(y_pred_gbr)): 
 acc_gbr.append(abs(y_pred_gbr[i]-Y_test[i])/Y_test[i]) final_s_gbr=sum(acc_gbr)/len(acc_gbr) 
  
acc_train_gbr=[] 
for i in range(0,len(y_pred_train_gbr)): 
 acc_train_gbr.append(abs(y_pred_train_gbr[i]-Y_train[i])/Y_train[i]) final_s_train_gbr=sum(acc_train_gbr)/len(acc_train_gbr) 
from sklearn.ensemble import RandomForestRegressor 
rfregressor=RandomForestRegressor(n_estimators=100,random_state=0) rfregressor.fit(X_train,Y_train) 
...
for i in range(0,len(y_pred_rfr)): 
 acc_rfr.append(abs(y_pred_rfr[i]-Y_test[i])/Y_test[i]) final_s_rfr=sum(acc_rfr)/len(acc_rfr) 
  
acc_train_rfr=[] 
for i in range(0,len(y_pred_train_rfr)): 
 acc_train_rfr.append(abs(y_pred_train_rfr[i]-Y_train[i])/Y_train[i]) final_s_train_rfr=sum(acc_train_rfr)/len(acc_train_rfr) 
...
for i in range(0,len(y_pred_svm)): 
 acc_svm.append(abs(y_pred_svm[i]-Y_test[i])/Y_test[i]) final_s_svm=sum(acc_svm)/len(acc_svm)
23 
acc_train_svm=[] 
for i in range(0,len(y_pred_train_svm)): 
 acc_train_svm.append(abs(y_pred_train_svm[i]-Y_train[i])/Y_train[i]) final_s_train_svm=sum(acc_train_svm)/len(acc_train_svm) 
...
final_pred=w1*rfregressor.predict(X_test)+w2*svmregressor.predict(X_test)+w3*gbregre ssor.predict(X_test) 
acc_test=[] 
...
print("Accuracy of gradient boost regression - ",(1-final_s_gbr)*100) print("Accuracy of random forest regression - ",(1-final_s_rfr)*100) print("Accuracy of support vector machine - ",(1-final_s_svm)*100) print("Accuracy of bagging model - ",(1-final_s__test_pred)*100) 
Y_final=Y 
print("Correlation of stock opening prices and oil prices - ") m1=np.mean(Y_final)*1258 
...
vary=0 
for i in range(0,1258): 
 varx=varx+Y_final[i].tolist()[0]*Y_final[i].tolist()[0]  vary=vary+list1_oil_price_add[i][0]*list1_oil_price_add[i][0] varx=varx*(1258)
26 
vary=vary*(1258) 
varx=varx-m1*m1 
vary=vary-m2*m2 
x_dev=math.sqrt(varx) y_dev=math.sqrt(vary) num=num/(x_dev*y_dev) print(num)
